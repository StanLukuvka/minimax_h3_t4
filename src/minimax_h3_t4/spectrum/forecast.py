"""Bounded Spectrum history forecaster.

Adapted from ComfyUI-Spectrum-MiniMax-H3 commit
85ec1da66277e893079ecd46e32cc865c56cfe53, copyright 2026 xmarre,
licensed GPL-3.0-or-later. Modified for direct worker-owned H3 execution.
"""
...
class HistoryWeightForecaster:
    ...
    def _ensure_factorization(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._factor_generation == self._generation and self._design is not None and self._cholesky is not None:
        ...
        *,
        ...
        for target_row, source_row in enumerate(resolved_rows):
            ...
            for offset in range(0, tail_numel, chunk_elements):
                ...
                self.last_prediction_max_fp32_elements = max(self.last_prediction_max_fp32_elements, accumulator.numel())
        return result
